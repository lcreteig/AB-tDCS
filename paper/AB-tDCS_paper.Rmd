...
We also recorded EEG during all three task blocks. Originally, we aimed to analyze the EEG data to obtain a neurophysiological correlate of the individual differences in response to tDCS [@Krause2014; @Li2015b; @Harty2017]. However, since we did not replicate the behavioral effect in @London2021, we refrained from further analysis of the EEG data. Instead, we are making the EEG data publicly available at this time [^EEG]. The dataset is formatted according to the new Brain Imaging Data Structure (BIDS) standard [@Gorgolewski2016] for EEG [@Pernet2018], to facilitate re-use. We include the raw data, as well as the fully preprocessed data and the MATLAB code that generated it.
...
Effects of tDCS at the group-level should manifest as a three-way interaction between Block, Stimulation, and Lag. As in @London2021, this interaction was not significant (`r apa_aov_T2.given.T1$statistic$block_stimulation_lag`). However, the higher-order interaction with Session order did reach significance (`r apa_aov_T2.given.T1$statistic$session_order_block_stimulation_lag`). This interaction appears to be mostly driven by a learning effect across sessions and blocks (the Session order by Stimulation by Lag interaction). From the first to the 2nd session, lag 3 performance improved, while lag 8 performance declined somewhat (leading to a smaller attentional blink). This change was stronger in participants that received anodal tDCS in the first session, and less pronounced in the cathodal-first group. We do not consider this a genuine effect of tDCS on the attentional blink, because there is no clear reason why these randomized groups should differ, and because the largest difference between the anodal and cathodal session occurred in the baseline block already (see Table \@ref(tab:tab-descriptives-T2)).
...
To evaluate whether the correlation in the present study was to be expected based on @London2021, we constructed a 95% _prediction interval_ (PI) [@Spence2016], using the correlation in @London2021 and the sample size of both studies (see the [Prediction interval] subsection of the [Methods] section).

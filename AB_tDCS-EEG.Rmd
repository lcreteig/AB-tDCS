...
plot_change_from_baseline <- function(df)
  ...
  scale_y_continuous("Change in AB magnitude (%)", breaks = seq(-40,40,10), limits = c(-40,40))
```
...
pcorr_change_baseline <- function(df) {
  df %>%
    ungroup() %>%
    mutate(first.session = as.numeric(first.session)) %>% # dummy code
    ...
    nest(first.session, AB.magnitude_baseline, AB.magnitude_change.score, T1.short_change.score) %>% 
    ...
    mutate(r = map_dbl(data, ~pcor(c("AB.magnitude_baseline","AB.magnitude_change.score",
                                     "first.session", "T1.short_change.score"), var(.)))) %>% 
...
df %>%
  ungroup() %>%
  select(-T1.short, -first.session) %>% # drop columns we don't need
...
```{r partial correlation anodal vs cathodal function}
...
df_study1_lag3 <- df_study1 %>%
  group_by(subject, first.session, block, stimulation) %>% # for each condition
  # calculate lag 3 as mean of 2 and 4; keep lag 10
  summarise(`3_T2.given.T1` = (T2.given.T1[lag == "2"] + T2.given.T1[lag == "4"]) / 2,
            `3_T1` = (T1[lag == "2"] + T1[lag == "4"]) / 2,
            `10_T2.given.T1` = T2.given.T1[lag == "10"], 
            `10_T1` = T1[lag == "10"]) %>%
  gather(condition, score, -subject, -first.session, -block, -stimulation) %>% # gather the 4 columns we created
  separate(condition, c("lag", "measure"), "_") %>% # separate lag and T1/T2|T1
  spread(measure, score) %>% # make a column of T1 and T2|T1
  mutate(lag = factor(lag)) #remake the lag factor
...
subject_info <- read_csv2(here("data", "subject_info.csv"), col_names = TRUE, progress = FALSE, col_types = cols(
  first.session = col_factor(c("anodal", "cathodal")),
